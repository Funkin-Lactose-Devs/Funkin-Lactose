...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		add(healthBar);
				...
				var grexEngineWatermark = new FlxText(4,FlxG.height - 4,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + " // Funkin' Lactose 0.1.0", 16);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			{
				// babyArrow.x -= 275;
				babyArrow.x -= 500; // if you cant remove them, put them offscreen
			}
			else
			{
				babyArrow.x -= 275;
	...
	{
		if(PreferencesMenu.getPref('hitsounds') && !note.isSustainNote)
